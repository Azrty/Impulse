 ...
 */
public class ImpulseLoadingOverlay extends LoadingOverlay {
    ...
    public ImpulseLoadingOverlay(final Minecraft mc, final ReloadInstance reloader, final Consumer<Optional<Throwable>> errorConsumer, Object displayWindow) {
        ...
        invoke(displayWindow, "addMojangTexture", new Class<?>[] { int.class }, mc.getTextureManager().getTexture(ResourceLocation.withDefaultNamespace("textures/gui/title/mojangstudios.png")).getId());
        this.progressMeter = StartupNotificationManager.prependProgressBar("Minecraft Progress", 1000);
    }

    public static Supplier<LoadingOverlay> newInstance(Supplier<Minecraft> mc, Supplier<ReloadInstance> ri, Consumer<Optional<Throwable>> handler, Object window) {
        return () -> new ImpulseLoadingOverlay(mc.get(), ri.get(), handler, window);
    }
    ...
    public void render(final GuiGraphics graphics, final int mouseX, final int mouseY, final float partialTick) {
        ...
        if (fadeouttimer >= 1.0F) {
            ...
            invoke(displayWindow, "render", new Class<?>[] { int.class }, 0xff);
        } else {
        ...
        final var twidth = contextInt("width");
        final var theight = contextInt("height");
        var wscale = (float) fbWidth / twidth;
        var hscale = (float) fbHeight / theight;
        var scale = contextInt("scale") * Math.min(wscale, hscale) / 2f;
        ...
        // top box from hpos
        addQuad(bufferbuilder, 0, fbWidth, wtop, fbHeight, fade);
        // bottom box to hpos
        addQuad(bufferbuilder, 0, fbWidth, 0, wtop, fade);
        // left box to wpos
        addQuad(bufferbuilder, 0, wleft, wtop, wbottom, fade);
        // right box from wpos
        addQuad(bufferbuilder, wright, fbWidth, wtop, wbottom, fade);
        ...
        RenderSystem.setShaderTexture(0, ((Number) invoke(displayWindow, "getFramebufferTextureId", new Class<?>[0])).intValue());
        ...
        if (fadeouttimer >= 2.0F) {
            ...
            invoke(this.displayWindow, "close", new Class<?>[0]);
        }
    ...
    private int contextInt(String method) {
        Object context = invoke(displayWindow, "context", new Class<?>[0]);
        return ((Number) invoke(context, method, new Class<?>[0])).intValue();
    }
    ...
    private static Object invoke(Object target, String method, Class<?>[] parameterTypes, Object... arguments) {
        try {
            ...
            throw new IllegalStateException("Impulse loading renderer handoff failed.", error);
        }
    }

    private static void addQuad(VertexConsumer bufferbuilder, float x0, float x1, float y0, float y1, float fade) {
        bufferbuilder.addVertex(x0, y0, 0f).setColor(0f, 0f, 0f, fade);
        bufferbuilder.addVertex(x0, y1, 0f).setColor(0f, 0f, 0f, fade);
        bufferbuilder.addVertex(x1, y1, 0f).setColor(0f, 0f, 0f, fade);
        bufferbuilder.addVertex(x1, y0, 0f).setColor(0f, 0f, 0f, fade);
    }
}
