...
//            int fade = 0xff;
    ...
    public static RenderElement impulseBrand(SimpleFont font) {
        Renderer text = (bb, ctx, frame) -> {
            int colour = ctx.colourScheme().foreground().packedint(globalAlpha);
            String wordmark = "IMPULSE";
            int left = ctx.scaledWidth() / 2 - (font.stringWidth(wordmark) + 58 * ctx.scale()) / 2 + 58 * ctx.scale();
            renderText(font, text(left, 164 * ctx.scale(), wordmark, colour), bb, ctx);
        };
        Renderer logo = initializeTexture("impulse-loading-logo.png", 128000, 2, (bb, ctx, size, frame) -> {
    ...
    public static RenderElement impulseStages(SimpleFont font) {
        return new RenderElement(() -> (bb, ctx, frame) -> {
            String current = currentLabel().toLowerCase();
            int active = current.contains("server") || current.contains("profile") ? 0
    ...
    private static String currentLabel() {
        ...
        ProgressMeter meter = activeMeter(meters);
        return meter == null ? "Preparing Minecraft" : meter.label().getText();
    }
    ...
    private static void arc(SimpleBufferBuilder bb, float x, float y, float radius, float thickness,
                            double start, double sweep, int colour) {
        int segments = Math.max(1, (int) Math.ceil(Math.abs(sweep) * 24));
        float inner = Math.max(0, radius - thickness);
        for (int i = 0; i < segments; i++) {
            double a = start + sweep * i / segments;
            double b = start + sweep * (i + 1) / segments;
            bb.pos(x + (float) Math.cos(a) * inner, y + (float) Math.sin(a) * inner).tex(0, 0).colour(colour).endVertex();
            bb.pos(x + (float) Math.cos(a) * radius, y + (float) Math.sin(a) * radius).tex(0, 0).colour(colour).endVertex();
            bb.pos(x + (float) Math.cos(b) * inner, y + (float) Math.sin(b) * inner).tex(0, 0).colour(colour).endVertex();
            bb.pos(x + (float) Math.cos(b) * radius, y + (float) Math.sin(b) * radius).tex(0, 0).colour(colour).endVertex();
        }
    }
