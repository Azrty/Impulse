...
public class SimpleFont {
    ...
    public SimpleFont(String fontName, int scale, int bufferSize, int textureNumber) {
        this(fontName, scale, bufferSize, textureNumber, 16);
    }

    public SimpleFont(String fontName, int scale, int bufferSize, int textureNumber, int logicalSize) {
        ...
        int fontSize = logicalSize * scale;
            ...
            try (STBTTPackRange.Buffer packRanges = STBTTPackRange.malloc(1)) {
                ...
                try (STBTTPackContext pc = STBTTPackContext.malloc()) {
                    ...
                    if (!stbtt_PackFontRanges(pc, buf, 0, packRanges)) {
                        throw new IllegalStateException("Loading font atlas is too small");
                    }
                    stbtt_PackEnd(pc);
