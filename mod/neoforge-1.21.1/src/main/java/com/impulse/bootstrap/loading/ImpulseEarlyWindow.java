 ...
 */
public class ImpulseEarlyWindow implements ImmediateWindowProvider {
    ...
    public Runnable initialize(String[] arguments) {
        ...
        fbScale = Math.max(2, Math.min(3, FMLConfig.getIntConfigValue(FMLConfig.ConfigValue.EARLY_WINDOW_FBSCALE)));
    ...
    private void initRender(final @Nullable String mcVersion, final String forgeVersion) {
        ...
        try {
            this.font = new SimpleFont("ImpulseSans.ttf", fbScale, 1000000, 1 + RenderElement.INDEX_TEXTURE_OFFSET);
        } catch (Throwable t) {
        ...
        this.elements = new ArrayList<>(Arrays.asList(
                RenderElement.impulseBrand(new SimpleFont("ImpulseSans.ttf", fbScale, 1000000, 3 + RenderElement.INDEX_TEXTURE_OFFSET, 38)),
                RenderElement.impulseStages(font),
                RenderElement.progressBars(font)));

        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glfwMakeContextCurrent(0);
        this.windowTick = renderScheduler.scheduleWithFixedDelay(this::renderThreadFunc, 0, 16, TimeUnit.MILLISECONDS);
     ...
     */
    synchronized void paintFramebuffer() {
    ...
    public void initWindow(@Nullable String mcVersion) {
        ...
        do {
            ...
            window = glfwCreateWindow(winWidth, winHeight, "Impulse", 0L, 0L);
            var erridx = versidx;
...
//        glfwSetWindowSizeLimits(window, 854, 480, GLFW_DONT_CARE, GLFW_DONT_CARE);
    ...
    public void updateModuleReads(final ModuleLayer layer) {
        var gameModule = layer.modules().stream()
                .filter(module -> module.getPackages().contains("com.impulse.modern121"))
                .findFirst().orElseThrow(() -> new IllegalStateException("Impulse runtime module was not found."));
        getClass().getModule().addReads(gameModule);
        var clz = Class.forName(gameModule, "com.impulse.modern121.ImpulseLoadingOverlay");
        if (clz == null) throw new IllegalStateException("Impulse loading overlay was not found in the runtime module.");
