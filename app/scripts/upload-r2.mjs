...
const RELEASE_CHANNEL = process.env.IMPULSE_RELEASE_CHANNEL === 'beta' ? 'beta' : 'stable'
...
async function main() {
  ...
  const files = readdirSync(DIST_DIR).filter((file) => {
    ...
    if (ext === '.yml' || ext === '.yaml') {
      const supported = RELEASE_CHANNEL === 'beta' ? ['beta'] : ['latest', 'beta']
      if (PLATFORM_FILTER === 'win') return supported.some((channel) => file === `${channel}.yml`)
      if (PLATFORM_FILTER === 'mac') return supported.some((channel) => file === `${channel}-mac.yml`)
      return supported.some((channel) => file === `${channel}.yml` || file === `${channel}-mac.yml`)
    }
  ...
  console.log(`Uploading ${files.length} ${RELEASE_CHANNEL} release file(s) to R2 bucket "${bucket}" for https://impulse.epivalent.com...\n`)
