...
function refreshLatestYmlForSignedExe() {
  ...
  let latest = readFileSync(latestPath, 'utf-8')
  latest = latest.replace(/(\n\s*-\s+url:\s+).*/i, `$1${exe}`)
  latest = latest.replace(/(\npath:\s+).*/i, `$1${exe}`)
  latest = latest.replace(/(\n\s+size:\s+)\d+/i, `$1${size}`)
  latest = latest.replace(/^(\s*sha512:\s*).+$/gm, `$1${sha512}`)
  writeFileSync(latestPath, latest)
  console.log('✓  Refreshed dist_electron/latest.yml with signed exe hash/size')
}
